	...
	<div>
		<el-row style="width: 100%; padding: 0.75rem 0; display: flex; justify-content: space-between; align-items: center;">
		    <el-col :span="8" style="text-align: left;margin-left: 1.875rem;">  
		      <p><el-button type="primary">实验条件2：纯文本编辑器+ChatGPT</el-button></p>  
			  <p><el-tag type="warning">右侧第一个蓝色图标为chatgpt</el-tag></p>
			  <p><el-tag type="warning">教案任务详细介绍请点击最右侧第2个蓝色图标</el-tag></p>
		...
		<!-- 	<el-col class="col01" :sm="1" :md="3" :lg="3" :xl="3">
						...
						<div>
							<el-container>
								<el-header><el-button type="primary" round size="small"
										class="title">ChatGPT</el-button></el-header>
								<el-main>
									<el-scrollbar wrap-class="message-list" wrap-style="height:30rem;" ref='scrollMenuRes'>
										<div v-for="(message, index) in messages" :key="index"
											...
											<div v-if="message.sender=='AI'" style="float:left;margin-right: 0.4rem;">
												<el-avatar :size="35" :src="img_chat"></el-avatar>
											</div>
											<div>
												...
												<div v-if="message.sender=='AI'"
													...
													<div>
														<p v-html="message.content" v-katex:auto></p>
			...
			<el-col :sm="1" :md="1" :lg="1" :xl="1" class="bg-right">
				<el-row>
					<div class="buttonAI" @click="clickBtnAI"><el-avatar :size="35" :src="img_chat"></el-avatar></div>
		...
		<el-dialog title="完成信息" :visible.sync="dialogForm4Visible">
		    ...
		    <p>填写网址：<a href="https://www.wjx.cn/vm/tbD4fkH.aspx" target="_blank">实验条件2 调查问卷</a> <p>
...
import {insertstudyorder,insertstudyorderc,insertstudtime,inserthistoryc,inserthistoryaia,getstudyorder,getstudyorderc} from '../api/api.js'
	...
	import imgProm from '@/assets/提示词.svg'
	//引入wangEditor编辑器
	import {
		Editor,
		Toolbar
	...
	export default {
		...
		data() {
			return {
				...
				prom:'你是一位，请你为老师设计一份优质且详细的教案，我将为你提供教学的基本信息以及教学模式。\n'+
                '教学基本信息:\n'+
                '年级科目：六年级数学\n'+
                '教学主题：圆的认识\n'+
                '教学模式-\n'+
		...
		mounted() {
			...
			this.options4=[`1-您在此实验条件中被分配的教案任务为:${this.matchT}`,'2-在此实验条件中，希望您能通过chatgpt进行生成一份教案，并且至少一次向chatgpt提问进行优化调整教案内容。',
			'3-点击 开始 按钮之后，会开始计时，直到点击完成 按钮之后，结束此次教案设计。',
			'4-您还可以参考我们的教学模式库或者自定义教学模式进行设计教学过程。'];
		...
		methods: {
			...
			handleTime(rate,index){
				...
				console.log(this.messages[index].rate+this.messages[index].time);
...
inserthistoryc('chatgpt', this.user_name , this.messages[index].content, sendTime ,'', this.messages[index].dtime,this.messages[index].rate, AcceptOrRejectTime).then(response =>{
					this.$message({
						message: 'inserthistoryaia chatgpt成功！',
						type: 'success'
						});
				})
				.catch(error => {
			...
			getLessonPlanName(){
				...
				// this.order=this.$route.params.order+1;//+1了，已老实
			...
			// 开始计时
				...
				// 3-完成实验需要记录实验数据
				insertstudtime('实验2',this.user_name,this.radio,this.timeGo,this.lessonSum,this.html).then(response =>{
					this.$message({
						message: '成功！',
						type: 'success'
						});
...
//console.log(this.user_name)
			//export const inserthistoryc = (Send_username , Receive_username , Content  , Sendtime  ,StartTime,EndTime, Rate, AcceptOrRejectTime ) => {
				inserthistoryc(this.user_name , 'chatgpt', this.inputMessage , formattedTime ,'','', 0, formattedTime).then(response =>{
					this.$message({
						message: 'inserthistoryc 用户成功！',
						type: 'success'
						});
				})
				.catch(error => {
			...
			async generateTextStreamToParagraph(us) {
				...
				this.cmessage.push({role: 'user',content: us});
				const url ="https://team3-at-2.openai.azure.com/openai/deployments/GPT35/chat/completions?api-version=2023-03-15-preview";
				...
				// 	})
				// }
				this.$set(this.messages[this.messages.length-1],'content',this.outcome);
                this.cmessage.push({role: 'assistant',content: this.outcome});
